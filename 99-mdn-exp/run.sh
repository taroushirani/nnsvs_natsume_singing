...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for s in ${testsets[@]}; do
        for typ in timelag duration acoustic; do
            checkpoint=$expdir/$typ/best_loss.pth
...
if [ ${stage} -le 99 ] && [ ${stop_stage} -ge 99 ]; then
	...
	for num_gaussians in 1 2 4 8 16; do
	    xrun nnsvs-train --config-dir hydra/${type}/train --config-path config.yaml \
		 ...
		 model=timelag train.out_dir=$expdir/timelag \
		 data.batch_size=$config_batch_size \
		 model.netG.num_gaussians=$num_gaussians | tee ./${type}_mix${num_gaussians}.log
	    ...
	    xrun nnsvs-train --config-dir hydra/${type}/train --config-path config.yaml \
		 ...
		 model=duration train.out_dir=$expdir/duration \
		 data.batch_size=$config_batch_size \
		 model.netG.num_gaussians=$num_gaussians |tee -a ./${type}_mix${num_gaussians}.log
	    xrun nnsvs-train \
		 data.train_no_dev.in_dir=$dump_norm_dir/$train_set/in_acoustic/ \
		 data.train_no_dev.out_dir=$dump_norm_dir/$train_set/out_acoustic/ \
		 data.dev.in_dir=$dump_norm_dir/$dev_set/in_acoustic/ \
